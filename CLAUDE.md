...
- **Cloud (always-on):** GitHub Actions `.github/workflows/worker.yml` — cron `*/5` + manual dispatch, `WORKER_KINDS=link_in_bio,video`. Video uses **Apify** (`_lib/apify.mjs` → direct video URL → `ffmpeg`), so no PC and no `yt-dlp` needed. `ffmpeg` is preinstalled on `ubuntu-latest`.
