...
- **Storage** bucket `recipe-images` (public read; authenticated insert/update/delete). Recipe cover images are downloaded and **re-hosted** here so they never expire — Instagram reel-cover CDN URLs carry an expiry signature, and source og:images can move. Core: `netlify/functions/_lib/images.mjs` (`rehostImage`). Sources by path: IG caption/reel cover from the **lookaside crawler page's `og:image`** (`fetchReelCover` — the embed's own `<img>` tags are the avatar + SUGGESTED posts, so picking the "largest" once grabbed a *different* reel's thumbnail; the lookaside page is keyed by the reel's `media_id`), website + link-in-bio from `og:image`, video reel from **Apify's cover image** (`displayUrl`). When an embed variant exposes no lookaside cover, the caption path queues a **`cover` job** (`recipe_jobs.kind='cover'`, `meta.recipe_id`) → the dispatch trigger runs the worker → it backfills a reliable cover from **Apify** and updates the row (fills in live in ~1–2 min; these jobs are hidden from the Library's job cards). Backfill existing rows with `scripts/backfill-images.mjs`; fix one with `scripts/refetch-cover.mjs <id>` (lookaside → Apify fallback).
